...
if API_TOKEN == "YOUR_BOT_TOKEN_HERE":
    print("WARNING: Set API_TOKEN before starting the bot.")

bot = telebot.TeleBot(API_TOKEN, parse_mode="HTML")

# Temporary state for the Channel Post Manager.
# This does not alter or replace any existing bot settings/features.
CHANNEL_POST_DRAFTS = {}
...
def add_user(user_id):
    with DB_LOCK:
        ...
        c = conn.cursor()
        c.execute(
            "INSERT OR IGNORE INTO users (user_id, status) VALUES (?, 'active')",
            (user_id,)
        )
        c.execute(
...
def handle_callbacks(call):
    ...
    # --------------------------------------------------------
    elif call.data == "menu_channel_post":
        show_post_channel_selector(chat_id, call.from_user.id)

    elif call.data.startswith("post_toggle_"):
        try:
            ch_db_id = int(call.data.replace("post_toggle_", ""))
        except ValueError:
            bot.answer_callback_query(call.id, "❌ Invalid channel")
            return

        draft = CHANNEL_POST_DRAFTS.setdefault(
            call.from_user.id,
            {"selected": set()}
        )
        selected = draft.setdefault("selected", set())
        if ch_db_id in selected:
            selected.remove(ch_db_id)
        else:
            selected.add(ch_db_id)

        bot.answer_callback_query(
            call.id,
            "✅ Selected" if ch_db_id in selected else "☑️ Unselected"
        )
        edit_post_channel_selector(call.message, call.from_user.id)

    elif call.data == "post_confirm_channels":
        draft = CHANNEL_POST_DRAFTS.get(call.from_user.id, {})
        selected = draft.get("selected", set())
        if not selected:
            bot.answer_callback_query(call.id, "❌ कम से कम 1 channel select करें", show_alert=True)
            return

        bot.answer_callback_query(call.id)
        CHANNEL_POST_DRAFTS[call.from_user.id] = {
            "selected": set(selected)
        }
        m = bot.send_message(
            chat_id,
            "📨 <b>अब post भेजें</b>\n\n"
            "जो भी पोस्ट करना है—text, photo, video, document, audio, sticker, animation आदि—"
            " इसी chat में भेजें।\n\n"
            "यह message selected सभी channels में copy होगा।"
        )
        bot.register_next_step_handler(
            m,
            receive_channel_post,
            call.from_user.id
        )

    elif call.data == "post_cancel":
        CHANNEL_POST_DRAFTS.pop(call.from_user.id, None)
        bot.answer_callback_query(call.id)
        show_admin_main(chat_id)
...
# CHANNEL POST MANAGER
# ============================================================
def get_channel_rows_by_ids(ids):
    if not ids:
        return []
    ...
    with DB_LOCK:
        ...
        c = conn.cursor()
        c.execute(
            f"SELECT id, channel_id, title, link FROM channels WHERE id IN ({placeholders})",
            tuple(ids)
        )
        rows = c.fetchall()
        conn.close()
...
def show_post_channel_selector(chat_id, user_id):
    CHANNEL_POST_DRAFTS[user_id] = {"selected": set()}
    rows = get_channels()
    ...
    if not rows:
        bot.send_message(
            ...
            "पहले <b>📺 चैनल → ➕ नया चैनल जोड़ें</b> से channel add करें।"
        )
        return
    ...
    bot.send_message(
        ...
        "एक या एक से ज्यादा channels select करें, फिर <b>Continue</b> दबाएँ।"
    )
    msg = bot.send_message(
        chat_id,
        "☑️ <b>Channels select करें:</b>",
        reply_markup=build_post_channel_markup(rows, set())
    )
    CHANNEL_POST_DRAFTS[user_id]["selector_message_id"] = msg.message_id

def build_post_channel_markup(rows, selected):
    markup = types.InlineKeyboardMarkup(row_width=1)
    for ch in rows:
        prefix = "✅" if ch[0] in selected else "⬜"
        title = parse_colors(ch[2])
        markup.add(types.InlineKeyboardButton(
            f"{prefix} {title}",
            callback_data=f"post_toggle_{ch[0]}"
        ))

    count = len(selected)
    markup.add(types.InlineKeyboardButton(
        f"➡️ Continue ({count} selected)",
        callback_data="post_confirm_channels"
    ))
    markup.add(types.InlineKeyboardButton(
        "❌ Cancel",
...
def edit_post_channel_selector(message, user_id):
    rows = get_channels()
    draft = CHANNEL_POST_DRAFTS.get(user_id, {})
    selected = draft.get("selected", set())
    try:
        bot.edit_message_reply_markup(
            message.chat.id,
            message.message_id,
            reply_markup=build_post_channel_markup(rows, selected)
        )
    except Exception:
        pass

def receive_channel_post(message, user_id):
    if not is_admin(message.from_user.id) or message.from_user.id != user_id:
        return

    draft = CHANNEL_POST_DRAFTS.get(user_id)
    if not draft or not draft.get("selected"):
        bot.send_message(ADMIN_ID, "❌ Post session expire हो गया। फिर से 📢 चैनल पोस्ट खोलें।")
        return
    ...
    m = bot.send_message(
        ADMIN_ID,
        "🔘 <b>Buttons add करें</b>\n\n"
        "हर button नई line में लिखें:\n"
        "<code>#g Button 1 | https://example.com</code>\n"
        "<code>#b Button 2 | https://t.me/example</code>\n"
        "<code>#r Button 3 | https://example.org</code>\n\n"
        "Buttons नहीं चाहिए तो <code>SKIP</code> भेजें।\n\n"
        "ℹ️ Telegram केवल supported button styles (#r/#g/#b) देता है; arbitrary HEX colors नहीं।"
    )
    bot.register_next_step_handler(
        m,
        finish_channel_post,
...
def finish_channel_post(message, user_id):
    if not is_admin(message.from_user.id) or message.from_user.id != user_id:
        return

    draft = CHANNEL_POST_DRAFTS.get(user_id)
    if not draft or "source_message_id" not in draft:
        bot.send_message(ADMIN_ID, "❌ Post session expire हो गया।")
        return

    selected_ids = set(draft.get("selected", set()))
    rows = get_channel_rows_by_ids(selected_ids)

    if not rows:
        CHANNEL_POST_DRAFTS.pop(user_id, None)
        bot.send_message(ADMIN_ID, "❌ Selected channels अब उपलब्ध नहीं हैं।")
        return

    button_text = "" if (message.text and message.text.strip().upper() == "SKIP") else (message.text or "")
    markup = build_keyboard_from_string(button_text) if button_text else None
    ...
    for ch in rows:
        ...
        title = parse_colors(ch[2])
        try:
            # Copy first, then attach the inline keyboard. This supports arbitrary
            # Telegram message content without changing the original message.
            ...
            failed.append(f"• {title}: {str(e)[:120]}")

    CHANNEL_POST_DRAFTS.pop(user_id, None)

    report = (
        "📢 <b>Channel Post Complete</b>\n\n"
        f"🟢 सफल: <b>{success}</b> / {len(rows)}\n"
    )
    if failed:
        report += "🔴 Failed: " + str(len(failed)) + "\n\n" + "\n".join(failed)
    else:
        report += "✅ सभी selected channels में post हो गया।"
...
def admin_reply_to_user(message):
    ...
    with DB_LOCK:
        ...
        c = conn.cursor()
        c.execute(
            "SELECT user_id FROM live_map WHERE admin_message_id=?",
            (replied_id,)
        )
        row = c.fetchone()
        conn.close()
